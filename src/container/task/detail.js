...
import { WhiteSpace, Badge, Button, Carousel, Toast } from 'antd-mobile'
...
export default class App extends Component {
  ...
  getDetail = async () => {
    let [err, res] = await axios.post(url.server + api.getCustomerDetail, {
        ...
        <Content style = { { paddingBottom: '1.2533rem' } }>
          ...
          >
            <Carousel
              ...
              { this.state.subscribeData.map((val, i) => (
                <SubscribeBox
                  key = { i }
                  data = { val }
                />
